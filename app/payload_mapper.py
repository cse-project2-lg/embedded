...
def sensor_raw_to_point(data: dict[str, Any]) -> Point:
    ...
    return (
        ...
        .field("sample_interval_ms", float(_safe_int(data.get("sampleIntervalMs"), 100)))
        .field("pir_motion", _safe_bool(sensors.get("pirMotion")))
        .field("pir_value", float(_safe_int(sensors.get("pirValue"))))
        .field("tof_distance_mm", float(_safe_int(sensors.get("tofDistanceMm"))))
        ...
        .field("wifi_rssi_dbm", float(_safe_int(transport.get("wifiRssiDbm"))))
